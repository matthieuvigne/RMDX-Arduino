// A test of using the RMD-X actuator in torque mode
...
void setup () {
    ...
    Serial.begin (38400) ;
    Serial.print("torque(10mA) target(10mA)");
    // Configure MCP2515
    SPI.begin () ;
...
uint32_t increaseTorqueDate = 0;
int32_t targetTorque = 50;
int32_t const MAX_TORQUE = 100.0; //Unit: 0.01A

void loop () {

  // Gradually increase motor target torque: +0.1A every 5s.
  if (increaseTorqueDate < millis())
  {
    increaseTorqueDate += 5000;
    targetTorque += 5;
  ...
  int currentTorque = motor.setTorque(1, targetTorque);

  Serial.print(currentTorque);
  Serial.print(" ");
  Serial.println(targetTorque);
  delay(50);
}
